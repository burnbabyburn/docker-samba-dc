...
# instead of enabling logs manually one could use supervisord logging feature; see supervisor config files
...
appSetup () {
  ...
  chown -R "${BINDUSERGROUP}":"${BINDUSERGROUP}" "${DIR_BIND9_LOG}/"
  chmod 770 "${DIR_BIND9_LOG}"

  #Fileperm on /etc/bind
  chown root:"${BINDUSERGROUP}" "${DIR_BIND9}"
  chmod 755 "${DIR_BIND9}"
...
#    set -- "$@" "--option=tls verify peer = ca_and_name"
  ...
  if [ ! -f "${FILE_SETUP_DONE}" ]; then
   # if [ -f "${FILE_SAMBA_CONF}" ]; then mv "${FILE_SAMBA_CONF}" "${FILE_SAMBA_CONF}".orig; fi
	...
	if [ -f "${FILE_BIND9_CONF_OPTIONS}" ]; then printf "include \"%s\";\n" "${FILE_BIND9_CONF_OPTIONS}" >> "${FILE_BIND9_CONF}"; fi
    if [ -f "${FILE_BIND9_CONF_DEF_ZONE}" ]; then printf "include \"%s\";\n" "${FILE_BIND9_CONF_DEF_ZONE}" >> "${FILE_BIND9_CONF}"; fi
    if [ -f "${FILE_BIND9_DNSROOT_KEY}" ]; then printf "include \"%s\";\n" "${FILE_BIND9_DNSROOT_KEY}" >> "${FILE_BIND9_CONF}"; fi

    ## Configure files generated by SAMBA setup
    # Add Debug to dynamically loadable zones (DLZ)
    if [ "${ENABLE_BIND9}" = true ] && [ -f "${FILE_BIND9_CONF_GEN_SAMBA}" ]; then 
    ...
    chown -R root:"${CHRONYUSERGROUP}" "${DIR_CHRONY_SOCK}"

    if [ ! -d "${DIR_SAMBA_CSHARE}" ]; then
	  mkdir "${DIR_SAMBA_CSHARE}"
	  mkdir "${DIR_SAMBA_ADMIN}"
	  mkdir "${DIR_SAMBA_ADMIN}/system32"
	  mkdir "${DIR_SAMBA_EVENTLOG}"
	  mkdir "${DIR_SAMBA_ADMIN}/system32/spool"
    fi
