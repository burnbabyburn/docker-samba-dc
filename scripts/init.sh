...
appFirstStart () {
  ...
  printf "smbclient: Connect as %s" "${DOMAIN_USER}" ; if ! smbclient "${SAMBADAEMON_DEBUG_OPTION}" -U"${DOMAIN_USER}%${DOMAIN_PASS}" -L LOCALHOST ; then printf "smbclient: Connect as %s FAILED" "${DOMAIN_USER}"; exit 1 ; fi
