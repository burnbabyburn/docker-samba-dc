...
RUN apt-get update \
    && apt-get upgrade -y \
	#openssl for dh key
    && apt-get install -y chrony pkg-config attr acl samba smbclient tdb-tools ldb-tools ldap-utils winbind libnss-winbind libpam-winbind libpam-krb5 krb5-user supervisor dnsutils \
